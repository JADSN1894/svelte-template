<script lang="ts">
	...
	// 			class = ` ${BG_COLOR['500'][primaryColor]} text-white font-medium rounded-full`;
	// 		} else if (dayjs(fullDay).isSame(period.start)) {
	// 			class = ` ${BG_COLOR['500'][primaryColor]} text-white font-medium ${
	...
	// 	(day: number) => {
	// 		let class = currentDateClass(day);
	...
	// 			class = ` ${BG_COLOR['100'][primaryColor]} ${currentDateClass(day)} dark:bg-white/10`;
	// 		}
	...
	// 			class = ` transition-all duration-500 text-white font-medium ${bgColor} ${
	...
	// 			return cn(
	// 				baseClass,
	// 				!activeDateData(day).active ? hoverClassByDay(day) : activeDateData(day).class,
